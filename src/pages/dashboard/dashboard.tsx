...
export const Dashboard = () => {
    ...
    const calculateStats = (shipmentsToStat: Shipment[]) => {
        ...
        const inTransit = shipmentsToStat.filter(s => s.status === 'em-trânsito' || s.status === 'agendado').length;
        const delivered = shipmentsToStat.filter(s => s.status === 'concluído').length;
        const pending = shipmentsToStat.filter(s => s.status === 'documentação').length;
